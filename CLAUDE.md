...
5. **실거래(실계좌) 연결 시 GitHub 레포를 반드시 비공개(private)로 되돌린다.** 현재는 모의투자라 공개(배포 편의). 실계좌 붙이는 순간부터 공개 금지 — 사용자에게 먼저 상기시킨다. 키는 공개/비공개 무관 항상 `.env`.

## 🛠 AI 작업 하네스 — 어떤 모델이든 코드 작업 전 이것부터
이 프로젝트는 사용자 혼자 오래 쓸 도구다. "빨리"보다 "안 망가뜨리기"가 우선.
1. **검증 게이트**: 코드를 바꿨으면 커밋 전 반드시 `bash scripts/check.sh` (가드 + 컴파일 + 단위테스트 22개). 실패 상태로 커밋·배포 금지.
2. **자동 가드**: 파일 편집 시 훅이 `scripts/guard.sh` 를 실행해 FDR 직접 import·실거래 코드·비밀키/개인데이터 추적을 즉시 잡는다. **가드를 끄거나 우회하지 말 것.**
3. **핵심 수식은 `tests/` 가 지킨다**: look-ahead 금지(t일 신호→t+1 체결)·손절·매도 재가중(선호 제외)·계좌 직렬화 하위호환·리포트 슬롯. **테스트가 깨지면 테스트를 고치기 전에 "왜 깨졌는지"부터 사용자에게 설명하라** — 대부분 코드가 잘못된 것이다.
...
- [x] **6단계: 24시간 배포** — Oracle Cloud 무료 VM(Ubuntu 22.04, Python 3.12)에 systemd 서비스로 상시 구동. 컴퓨터 꺼도 24시간 자동운용. `deploy/` 스크립트 + DEPLOY.md 참고
