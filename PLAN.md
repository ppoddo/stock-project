...
- [x] `trading/backtest/` 엔진 추가 (소스 추상화 재사용)
- [x] 4대 지표 산출: **CAGR · MDD · Sharpe · 승률** + Buy&Hold 비교
- [x] 추세 점수 룰 검증: "추세 ≥ 매수선 보유 / ≤ 매도선 현금"
- [x] look-ahead bias 차단(t일 신호 → t+1 체결), 거래비용 반영
- [x] 대시보드에 백테스트 탭 추가 (임계값 슬라이더 + 자산곡선)
- [ ] (남음) 매수/매도 임계값 그리드 스윕으로 최적값 탐색
- [ ] (남음) 뉴스·선호 가중치 튜닝 — 단 과거 뉴스 재현 한계로 별도 설계 필요

> 📌 검증 결과: 단순 추세추종은 **강세장에선 B&H에 지지만 변동성·하락장(네이버·테슬라)에선 이긴다**.
> 승률 32~49%(추세추종 특성). MDD가 크므로 포지션 사이징·리스크 관리가 다음 과제.
